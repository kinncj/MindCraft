...
export const TOOL_TEMPLATES: Record<string, string> = {
  ...
  build_feature: '{"tool":"build_feature","args":{"kind":"treehouse","width":7,"length":7,"color":"color_pink"}}  (one thing on its own: bridge|treehouse|playground|court|garden|fountain|parking|fence)',
  build_dig: '{"tool":"build_dig","args":{"kind":"lake","width":30,"length":20,"depth":3}}  (dig: pool|raisedPool|lake|pond|pit|bunker|tunnel|well|moat)',
...
export const PROMPT_EXAMPLES: Array<{ user: string; say: string; actions: string }> = [
  ...
  { user: 'a school with 6 classrooms, a computer room, a sports court, students and teachers, and a playground', say: 'School time! 🏫 Classrooms, computers, a court, and a playground!', actions: '[{"tool":"build_house","args":{"type":"school","floors":2,"wall":"brick","trim":"color_yellow","furnish":true,"roomPlan":[{"purpose":"classroom","count":6},{"purpose":"computer room","count":1}],"features":["court","playground"]}},{"tool":"villager_spawn","args":{"job":"teacher"}},{"tool":"villager_spawn","args":{"job":"random","name":"Student Sam"}}]' },
  { user: 'build a big treehouse', say: 'A treehouse! 🌳 Up the ladder we go!', actions: '[{"tool":"build_feature","args":{"kind":"treehouse","width":7,"length":7}}]' },
  { user: 'make a long bridge over the water', say: 'A long bridge! 🌉 Mind the railings!', actions: '[{"tool":"build_feature","args":{"kind":"bridge","width":21,"length":5}}]' },
  { user: 'dig a 30 by 20 lake', say: 'A big lake! ⛏️💧 Watch me dig!', actions: '[{"tool":"build_dig","args":{"kind":"lake","width":30,"length":20,"depth":3}}]' },
...
export class WebLlmProvider implements ChatProvider {
  ...
  systemPrompt(ctx: ChatContext): string {
    ...
    return [
      `You are ${v.name}, a cheerful ${v.jobLabel}${v.gender === 'girl' ? ' (she/her)' : v.gender === 'boy' ? ' (he/him)' : ''} in a friendly block-building game for small kids.`,
